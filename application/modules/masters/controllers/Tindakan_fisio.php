...
class Tindakan_fisio extends MX_Controller 
{
	...
	public function index() {
		$data['title'] 			= 'Master Tindakan Fisio';
		$data['view'] 			= 'list_tindakan_fisio';
		$data['allTindakan']	= $this->m_tindakan_fisio->getAllTindakan();
		$this->load->view('templates_backend',$data);
	}
	
	function detail($id_tindakan_master)
	{
		$data['title'] 			= 'Detail Tindakan Fisioterapis';
		$data['view'] 			= 'detail_tindakan_fisio';
		$data['get_detail'] 	= $this->m_tindakan_fisio->getDetail($id_tindakan_master);
		$this->load->view('templates_backend',$data);
	}
	...
	function save()
	{
		...
		$this->form_validation->set_rules('nama_tindakan', 'Nama Tindakan Fisio', 'required');
		...
		$this->m_tindakan_fisio->input($data, 'tb_tindakan_master');
		
		$notif = array(
						'title' => 'Berhasil, Data Anda Telah di Input !!!', 
						'message' => 'Success', 
						'status' => 1,
						'redirect' => ''.base_url().'masters/tindakan_fisio'
					);
	...
	function update()
	{
		...
		$this->m_tindakan_fisio->update('tb_tindakan_master','id_tindakan_master', $id_tindakan_master, $data);
	...
	function delete($id_tindakan_master)
	{
		...
		redirect('masters/tindakan_fisio');
	}
}