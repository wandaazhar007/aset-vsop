...
<div class="panel panel-flat">
	<div class="panel-heading">
		<h5 class="panel-title">List Pendaftaran Pasien</h5>
	...
	<table class="table datatables-lists">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<?php echo $this->mrclibs->statusRegistrasi($p->status);?>
				</td>
					...
					<ul class="icons-list">
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-menu-right">
								<?php
									$tombol_tindakan = '<li><a href="'.base_url().'tindakan/input/'.$p->id_registrasi.'/'.$p->no_rm.'">Lakukan Tindakan</a></li>';
									$p->status == '0' ? $tbt = $tombol_tindakan : $tbt = '';
								?>
								<?php echo $tbt;?>
								
								<li><a href="<?php echo base_url();?>registrasi/detail/<?php echo $p->id_registrasi;?>">Edit</a>
								</li>
								<li><a href="<?php echo base_url()?>registrasi/delete/<?php echo $p->id_registrasi;?>" onClick="return confirm('Apakah Anda yakin ingin Menghapus Data Pasien Ini ?')">Delete</a>
								</li>
							</ul>
						</li>
					</ul>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('.datatables-lists').DataTable({
			buttons: [
				{
					text: '<i class="icon-plus-circle2"></i>  Pasien Baru',
					className: 'btn bg-success-400',
					action: function(e, dt, node, config) {
						window.location.href = "<?php echo base_url('registrasi/pasien_baru');?>";
					}
				},
				{
					text: '<i class="icon-plus-circle2"></i>  Pasien Lama',
					className: 'btn bg-primary-400',
					action: function(e, dt, node, config) {
						window.location.href = "<?php echo base_url('registrasi/pasien_lama');?>";
					}
				}
			]
		});
